...
app.service('api', ['$http', '$rootScope', function ($http, $rootScope) {
  ...
  this.handleAuthentication = function (result) {
    ...
    if (result.avatar == 'null' || 'default.jpg') $rootScope.user.img = "https://assets.beep.nl/avatars/default.jpg";else $rootScope.user.img = result.avatar; // token
...
app.service('inspections', ['$http', '$rootScope', 'api', 'settings', function ($http, $rootScope, api, settings) {
  ...
  this.newSaveObject = function () {
    ...
    if (data == null && init == false) data = self.inspection;
  ...
  this.loadRemoteInspections = function (hive_id) {
    if (_typeof(hive_id) != undefined && hive_id != null) api.getApiRequest('inspections', 'inspections/hive/' + hive_id);
  };
  ...
  this.inspectionHandler = function (e, data) {
    ...
    if (typeof data.checklist_id != 'undefined') self.checklistId = data.checklist_id;else self.checklistId = null;
...
app.controller('InspectionCreateCtrl', function ($scope, $rootScope, $window, $location, $filter, $routeParams, $timeout, settings, api, moment, hives, groups, inspections) {
  ...
  $scope.editChecklist = function () {
    var so = $scope.inspection; //console.log('editChecklist', so);
  ...
  $scope.selectChecklist = function (id) {
    ...
    var updateInspection = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  $scope.saveBeforeNavigate = function () {
    var callback = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    var memory = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    $scope.memory = memory;
  ...
  $scope.back = function () {
    if ($rootScope.optionsDialog) {
      ...
      for (var i = $rootScope.history.length - 1; i >= 0; i--) {
        var path = $rootScope.history[i];
        var go = false;
        var hive_id = typeof $scope.hive != 'undefined' && $scope.hive != null ? $scope.hive.id : ''; // go if history item contains main menu item

        if (path.indexOf('/inspections') > -1 && path.indexOf('/inspections/') == -1 || path.indexOf('/locations') > -1 || path.indexOf('/hives') > -1 && path.indexOf('/hives/' + hive_id) == -1 && path.indexOf('/inspect') == -1 || path.indexOf('/groups') > -1) go = true;
...
app.controller('InspectionsCtrl', function ($scope, $rootScope, $window, $location, $filter, $routeParams, settings, api, moment, hives, inspections, groups) {
  ...
  $scope.init = function () {
    if (api.getApiToken() == null) {
      ...
      if ($scope.hive == null) return $location.path('/locations');
...
app.controller('ResearchesCtrl', function ($scope, $rootScope, $window, $timeout, $location, $filter, $interval, api, $routeParams, ngDialog, hives, measurements) {
  ...
  $scope.selectedConsentId = null; // handlers
  ...
  $scope.setDateLanguage = function () {
    ...
    var maxDate = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    $("#dtBox").DateTimePicker({
      minDateTime: minDate,
      maxDateTime: maxDate,
      dateTimeFormat: 'yyyy-MM-dd HH:mm',
  ...
  $scope.updateConsent = function (research_id, consent_id, date) {
    ...
    api.patchApiRequest('researchConsent', 'research/' + research_id + '/edit/' + consent_id, {
      'updated_at': date
    });
  };
